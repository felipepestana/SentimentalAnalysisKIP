...
med_pos = statistics.mean(list_pos)
med_neg = statistics.mean(list_neg)
print(med_pos)
print(med_neg)

for seed in final_dict:
    if final_dict[seed][2] == 0:
        print(final_dict[seed][0])
        if 'positive' in final_dict[seed][0]:
            final_dict[seed][2] = med_pos
        else:
            final_dict[seed][2] = med_neg

with open('dict_cut.csv', 'w', newline='', encoding='utf-8') as csv_result:
