...
def clearTel(body):
    ...
    clearText = re.sub(r'(\(?\d{2}\)?)?[-\s]?\d{4}[ -]\d{4}', '', body)
    return clearText
...
i = 0
with open('2.Tcc_History.csv', encoding='utf-8') as csvfile:
    with open('Tcc_Clear.csv', 'w', newline='', encoding='utf-8') as csv_result:
        ...
        for row in reader:
            ...
            if i == 0:
                ...
                result = remove_signature(result)
                result = remove_break_line(result)
                if re.search('[a-zA-Z]', result) and len(result) > 1:                    
